 ...
 */
    ...
    $description_update = "UPDATE `temp_monitor_online_payment` SET `description`='".$Description."', `res_code` = '".$ResponseCode."' WHERE (`transaction_id`='".$TransactionId."') ";
    $descriptiong_run = $conn->query($description_update);

   
    $query_track_pay = mysqli_query($conn, "SELECT * FROM temp_monitor_online_payment WHERE transaction_id = '".$TransactionId."' ORDER BY id DESC LIMIT 1");
    ...
    $vote_count = 0;      
    if($amount_billed == 1.2)
    {
        $vote_count   = 2;
    }elseif($amount_billed == 12)
    {
        $vote_count   = 20;
    }elseif($amount_billed == 6)
    {
        $vote_count   = 10;
    }elseif($amount_billed == 30)
    {
        $vote_count   = 50;
    }elseif($amount_billed == 60)
    {
        $vote_count   = 100;
    }elseif($amount_billed == 120)
    {
        $vote_count   = 200;  
    }elseif($amount_billed == 300) 
    {
        $vote_count   = 500;
    }elseif($amount_billed == 600) 
    {
        $vote_count   = 1000;
    }                        
    ...
    $query_nominee_web = mysqli_query($conn, "SELECT * FROM channel_votes WHERE `id` = '".$contestant_code."' LIMIT 1");
    while($rowweb = mysqli_fetch_assoc($query_nominee_web))
    {
        $old_voteweb    = $rowweb['web'];
    }

    $new_web_vote    = $old_voteweb  + $vote_count;
    ...
    if($ResponseCode == "0000") 
    {
        ...
        $update_contestant = "UPDATE contestants_weekly SET num_of_votes = '".$new_vote."' WHERE contestant_id = '".$contestant_code."'";
        $begin_update = $conn->query($update_contestant);


        $update_contestantweb = "UPDATE channel_votes SET web = '".$new_web_vote."' WHERE id = '".$contestant_code."'";
        $begin_updateweb = $conn->query($update_contestantweb);

        

        //SUM UP ALL IN ONE TABLE IF SUCCESSFUL
        $when = date("Y-m-d H:i:s");
        $gmb_query = "INSERT INTO diasa_pay(`transaction_id`, `amt_after_charges`, `client_reference`, `contestant_name`, `external_trans_id`, `number`, `device`, `channel`, `number_of_votes`, `amount`, `response_code`, `charges`, `description`, `when`) VALUES('".$json['Data']['TransactionId']."', '".$AmountAfterCharges."', '".$ClientReference."', '".$contestant_name."', '".$ExternalTransactionId."', '".$voting_number."', 'web', 'momo', '".$vote_count."', '".$amount_billed."', '".$ResponseCode."', '".$Charges."', '".$Description."', '".$when."')";                
    ...
    if($ResponseCode == "0000") 
    {
        $message = "Voting processed successfully.\nThank you and keep voting.\nPowered by www.mobilecontent.com.gh . Reach us for Mobile App & USSD, Value Added Services, Software, etc";
    ...
    {
        $message = "Sorry voting failed.\n".$description.". Thank you!.\nPowered by www.mobilecontent.com.gh . Reach us for Mobile App & USSD, Software, Value Added Services, Software, etc";
