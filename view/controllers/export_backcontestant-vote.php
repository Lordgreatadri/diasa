<?php
  ...
  include_once "db-config.php";
      ...
      $filename = "../data_exports/".date('Y').'-'.strtotime("").'secondChcontestants_'.date('d-M').'.csv';

    $today_date = date('Y-m-d');
      
          #*************         SQL SELECT STATEMENT        *********************************
    $sql = "SELECT contestant_id AS CODE, name AS 'CONTESTANT NAME', status AS STATUS,  num_of_votes AS VOTES FROM contestants_weekly WHERE backlist = 'True' ORDER BY num_of_votes DESC";//s.date_unsub = '$today_date'
    ...
    if($num_rows >= 1)
    {
      ...
      echo "<p style='text-align:center; margin-top: 200px;'>Your file is ready. You can download it from <a href='$filename'>here!</a>". ' or open file location to view in folder. <a href = "../backlist.php"> [ Back <<< ]</a> </p>';
    }
    else
    {
      echo "<p style='text-align:center; margin-top: 200px;'>There is no matching record found for your criteria sort <a href = '../backlist.php'>[ Back<<< ]</a></p>";
    }
