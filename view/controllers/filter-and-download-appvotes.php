...
if($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    {
        //query to get the categories
        $query = "SELECT `t`.`number`,`t`.`channel`,`t`.`amount`,`t`.`nominee_name`,`t`.`when`,`m`.`number_of_votes`, `m`.`client_reference` FROM track_pay t INNER JOIN diasa_pay m ON `m`.`transaction_id` = `t`.`transac_id` WHERE `t`.`number` LIKE '%$filterKey%' OR `t`.`nominee_name` LIKE '%$filterKey%' AND `m`.`client_reference` = '23214' AND response_code = '0000' ORDER BY `m`.`when` DESC";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $filename = "app-criteria_Data.csv";
        ...
        //query to get the categories
        $query = "SELECT `t`.`number`,`t`.`channel`,`t`.`amount`,`t`.`nominee_name`,`t`.`when`,`m`.`number_of_votes`, `m`.`client_reference` FROM track_pay t INNER JOIN diasa_pay m ON `m`.`transaction_id` = `t`.`transac_id` WHERE (DATE(`m`.`when`) BETWEEN '$startDate' AND '$endDate') AND `m`.`client_reference` = '23214' AND response_code = '0000' ORDER BY `m`.`when` DESC ";
        ...
        //query to get the categories
        $query = "SELECT `t`.`number`,`t`.`channel`,`t`.`amount`,`t`.`nominee_name`,`m`.`when`,`m`.`number_of_votes`, `m`.`client_reference` FROM track_pay t INNER JOIN diasa_pay m ON `m`.`transaction_id` = `t`.`transac_id` WHERE `m`.`client_reference` = '23214' AND response_code = '0000' ORDER BY `m`.`when` DESC";
