<?php
  ...
  include_once "db-config.php";
      ...
      $filename = "../data_exports/".date('Y').'-'.strtotime("").'contestants_votes_'.date('d-M').'.csv';

    $today_date = date('Y-m-d');
      
          #*************         SQL SELECT STATEMENT        *********************************
    $sql = "SELECT contestant_id AS CODE, name AS 'CONTESTANT NAME', status AS STATUS,  num_of_votes AS VOTES FROM contestants_weekly WHERE status = 'not_evicted' AND type = 'single' ORDER BY num_of_votes DESC";//s.date_unsub = '$today_date'
