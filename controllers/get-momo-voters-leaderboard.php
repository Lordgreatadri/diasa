<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        $query = "SELECT DISTINCT `number` as voter_num, SUM(number_of_votes) as vote_num FROM `miss_gh_pay` WHERE response_code = '0000' AND channel = 'momo'  GROUP BY `number` ORDER BY vote_num DESC LIMIT 5";//OR `channel` = 'mtn-gh' OR `channel` = 'vodafone-gh' OR `channel` = 'airtel-gh' OR `channel` = 'tigo-gh'
